...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save') {
        ...
        $data = ['name'=>$name,'slug'=>$slug,'is_active'=>isset($_POST['is_active'])?1:0,'description'=>trim(sanitize($_POST['description']??''))];
        if ($id) { $db->update('gallery_categories',$data,'id=?',[$id]); }
        else     { $db->insert('gallery_categories',$data); }
...
$cats   = $db->fetchAll("SELECT gc.*, COUNT(gi.id) as img_count FROM gallery_categories gc LEFT JOIN gallery_images gi ON gi.category_id=gc.id GROUP BY gc.id ORDER BY gc.name");
...
<div style="display:grid;grid-template-columns:1fr 340px;gap:1.5rem;align-items:start;">
  <div style="background:#fff;border:1.5px solid #E5E7EB;border-radius:12px;overflow:hidden;">
    <div style="padding:1.1rem 1.5rem;border-bottom:1px solid #E5E7EB;font-weight:700;font-size:.88rem;"><?= count($cats) ?> Categories</div>
    <table style="width:100%;border-collapse:collapse;">
      <thead><tr style="background:#F8FAF9;border-bottom:1px solid #E5E7EB;">
        <th style="padding:.6rem 1.25rem;text-align:left;font-size:.72rem;font-weight:700;color:#9CA3AF;text-transform:uppercase;">Name</th>
        <th style="padding:.6rem 1.25rem;text-align:center;font-size:.72rem;font-weight:700;color:#9CA3AF;text-transform:uppercase;">Images</th>
        <th style="padding:.6rem 1.25rem;text-align:center;font-size:.72rem;font-weight:700;color:#9CA3AF;text-transform:uppercase;">Active</th>
        <th style="padding:.6rem 1.25rem;text-align:right;"></th>
      </tr></thead>
      <tbody>
        <?php foreach ($cats as $c): ?>
        <tr style="border-bottom:1px solid #F0F0F0;" onmouseover="this.style.background='#FAFAFA'" onmouseout="this.style.background=''">
          <td style="padding:.75rem 1.25rem;font-weight:600;font-size:.85rem;"><?= htmlspecialchars($c['name']) ?></td>
          <td style="padding:.75rem 1.25rem;text-align:center;font-size:.82rem;"><?= $c['img_count'] ?></td>
          <td style="padding:.75rem 1.25rem;text-align:center;font-size:.78rem;font-weight:700;color:<?= $c['is_active']?'#065F46':'#9CA3AF' ?>;"><?= $c['is_active']?'Yes':'No' ?></td>
          <td style="padding:.75rem 1.25rem;text-align:right;">
            <div style="display:flex;gap:.4rem;justify-content:flex-end;">
              <a href="?edit=<?= $c['id'] ?>" style="padding:.3rem .5rem;border-radius:6px;background:#EFF6FF;color:#3B82F6;font-size:.72rem;text-decoration:none;">Edit</a>
              <?php if ($c['img_count'] == 0): ?>
              <form method="POST" onsubmit="return confirm('Delete?');" style="display:inline;">
                <input type="hidden" name="action" value="delete">
                <input type="hidden" name="id" value="<?= $c['id'] ?>">
                <button type="submit" style="padding:.3rem .5rem;border-radius:6px;background:#FEF2F2;color:#EF4444;border:none;cursor:pointer;font-size:.72rem;">Del</button>
              </form>
              <?php endif; ?>
            </div>
          </td>
